...
%code{
    ...
    void copyParsedCodes(nate::NateParser& aNate)
    {
...
for-loop-statement:
	...
	id 
		{ 
            nate.data.forId = $id;
            lexer.popState();
		    lexer.stateExprValue = nate::Lexer::FOR_EXPR;
        }
	for-part
        { 
            lexer.stateExprValue = nate::Lexer::EXPR;
        }
	for-loop-part-end
  ;
...
expr-non-word:
	NUMBER
		{ 
            ...
            {
                error(@NUMBER, std::string("Number " + $NUMBER + " is not supported (yet)"));
            }
		}
  | RATIONAL
		{ 
...
void nate::parser::error(const location& loc, const std::string& msg)
{
